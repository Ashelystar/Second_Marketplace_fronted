  ...
  <div class="bg-white min-h-screen">
    <header class="bg-white sticky top-0 z-50 shadow-sm">
      <div class="max-w-[1600px] mx-auto px-4 py-3 flex items-center">
        ...
        <div class="flex-1 relative max-w-2xl">
          ...
          <button class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-xianyuText" @click="performSearch">
    ...
    <div class="fixed inset-0 bg-black/50 z-50" :class="{ 'hidden': !filterModalOpen }" @click="filterModalOpen = false">
      <div class="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[80vh] overflow-y-auto p-4" @click.stop>
        ...
        <div class="space-y-6">
          ...
          <div><h4 class="font-medium mb-3">发布时间</h4>
            <div class="flex flex-wrap gap-2">
              <button v-for="t in timeRanges" :key="t.value" class="filter-tag" :class="{ 'active': filterState.timeRange === t.value }" @click="filterState.timeRange = filterState.timeRange === t.value ? '' : t.value">{{ t.label }}</button>
            </div>
          </div>
    ...
    <div class="fixed right-4 top-1/2 -translate-y-1/2 z-30 hidden lg:block">
      <div class="flex flex-col gap-2">
        <button v-for="t in floatingTools" :key="t.id" class="w-12 h-12 bg-white rounded-full shadow-lg flex items-center justify-center hover:bg-gray-50 transition-all group relative" @click="t.action()">
          <i :class="t.icon"></i>
...
const filterState = ref({ minPrice: '', maxPrice: '', conditions: [] as string[], locations: [] as string[], timeRange: '' })
...
const sortOptions = [
  ...
  { value: 'credit-high' as SortOption, label: '信用排序', icon: 'fa fa-credit-card mr-2 text-gray-400' },
